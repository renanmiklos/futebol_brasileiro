...
// Função reutilizável para montar e executar a query SQL genérica para clubes da Região Sudeste
if (!function_exists('build_sql_query_sudeste')) {
    function build_sql_query_sudeste($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem) {
        $campos = "
            t.nome AS nome_time,
            t.escudo,
            t.estado,
        ...
        if ($tituloNormalizado === 'sudeste na série a - pontos corridos') {
            $campos .= ", SUM(c.pontos_marcados) AS pontos_marcados";
        } elseif ($tituloNormalizado === 'sudeste na série b - pontos corridos') {
            $campos .= ", SUM(c.pontos_marcados) AS pontos_marcados";
        }
        // Outros títulos podem ser adicionados aqui se necessário

        // Filtro pelos estados da Região Sudeste
        $where = " t.estado IN ('SP', 'RJ', 'MG', 'ES')";
...
// Função reutilizável para montar e executar a query SQL genérica para participações da Região Sudeste
if (!function_exists('build_sql_query_participacoes_sudeste')) {
    function build_sql_query_participacoes_sudeste($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem) {
        // Siglas dos estados da Região Sudeste
        $siglas_sudeste = ['SP', 'RJ', 'MG', 'ES'];

        // Preparar named placeholders para IN (...)
        $estado_placeholders = [];
        foreach ($siglas_sudeste as $i => $sigla) {
...
// Função para processar os cases da Região Sudeste
function processar_sudeste($tituloNormalizado, &$descricao, &$dados, &$id_competicao, &$ano_inicio, &$exibir_tabela, &$tipo_participacao, &$tabela_estatisticas, $pdo, $coluna_ordem, $tipo_ordem) {
    $retorno = false; // Indica se algum case sudeste foi encontrado

    switch ($tituloNormalizado) {
        // Casos para Participações da Região Sudeste
        case 'participações do sudeste na libertadores':
            $descricao = "Participações dos clubes da Região Sudeste na Copa Libertadores da América.";
            $id_competicao = 5;
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_sudeste';
            $retorno = true;
            break;
        case 'participações do sudeste na sul americana':
            $descricao = "Participações dos clubes da Região Sudeste na Copa Sul Americana.";
            $id_competicao = 7;
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_sudeste';
            $retorno = true;
            break;
        case 'participações do sudeste - série a':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série A.";
            $id_competicao = 19; // Série A
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_sudeste';
            $retorno = true;
            break;
        case 'participações do sudeste - série a - pontos corridos':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série A 
            ...
            $retorno = true;
            break;
        case 'participações do sudeste - série b':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série B.";
            $id_competicao = 20; // Série B
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_sudeste';
            $retorno = true;
            break;
        case 'participações do sudeste - série b - pontos corridos':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série B 
            no período dos Pontos Corridos.";
            $id_competicao = 20;
            $ano_inicio = 2006;
            ...
            $retorno = true;
            break;
        case 'participações do sudeste - série c':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série C.";
            $id_competicao = 21; // Série C
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_sudeste';
            $retorno = true;
            break;
        case 'participações do sudeste - série d':
            $descricao = "Participações dos clubes da Região Sudeste no Campeonato Brasileiro Série D.";
        ...
        // Casos para Resultados da Região Sudeste (resultados gerais)
        case 'sudeste na libertadores':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Copa Libertadores da América.";
            $id_competicao = 5;
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'sudeste na sul americana':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Copa Sul Americana.";
            $id_competicao = 7;
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'sudeste na série a':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série A do Campeonato Brasileiro.";
            $id_competicao = 19; // Série A
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'sudeste na série a - pontos corridos':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série A do Campeonato Brasileiro 
            no período dos pontos corridos.";
            $id_competicao = 19;
            $ano_inicio = 2003;
            ...
            $retorno = true;
            break;
        case 'sudeste na série b':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série B do Campeonato Brasileiro.";
            $id_competicao = 20; // Série B
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'sudeste na série b - pontos corridos':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série B do Campeonato Brasileiro 
            no período dos pontos corridos.";
            $id_competicao = 20;
            $ano_inicio = 2006;
            ...
            $retorno = true;
            break;
        case 'sudeste na série c':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série C do Campeonato Brasileiro.";
            $id_competicao = 21; // Série C
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'sudeste na série d':
            $descricao = "Estatísticas gerais dos clubes da Região Sudeste na Série D do Campeonato Brasileiro.";
            $id_competicao = 22; // Série D
            $exibir_tabela = true;
            $retorno = true;
            break;
    ...
    // Executa a consulta específica para participações da Região Sudeste antes da consulta genérica de resultados
    if ($retorno && !empty($id_competicao) && $exibir_tabela && $tipo_participacao === 'participacoes_sudeste') {
        $tabela_estatisticas = build_sql_query_participacoes_sudeste($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem);
    }
    // Executa a consulta genérica apenas se for um case de resultados da Região Sudeste e não for do tipo participacoes
    elseif ($retorno && !empty($id_competicao) && $exibir_tabela && $tipo_participacao === null) {
        $tabela_estatisticas = build_sql_query_sudeste($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem);
    }

    return $retorno; // Retorna true se o item foi um case da Região Sudeste, false caso contrário
}
?>