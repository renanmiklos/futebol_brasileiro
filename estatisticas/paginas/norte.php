...
// Função reutilizável para montar e executar a query SQL genérica para clubes da Região Norte
if (!function_exists('build_sql_query_norte')) {
    function build_sql_query_norte($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem) {
        $campos = "
            t.nome AS nome_time,
            t.escudo,
            t.estado,
        ...
        if ($tituloNormalizado === 'norte na série a - pontos corridos') {
            $campos .= ", SUM(c.pontos_marcados) AS pontos_marcados";
        } elseif ($tituloNormalizado === 'norte na série b - pontos corridos') {
            $campos .= ", SUM(c.pontos_marcados) AS pontos_marcados";
        }
        // Outros títulos podem ser adicionados aqui se necessário

        // Filtro pelos estados da Região Norte
        $where = " t.estado IN ('PA', 'AM', 'AP', 'TO', 'AC', 'RR', 'RO')";
...
// Função reutilizável para montar e executar a query SQL genérica para participações da Região Norte
if (!function_exists('build_sql_query_participacoes_norte')) {
    function build_sql_query_participacoes_norte($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem) {
        // Siglas dos estados da Região Norte
        $siglas_norte = ['PA', 'AM', 'AP', 'TO', 'AC', 'RR', 'RO'];

        // Preparar named placeholders para IN (...)
        $estado_placeholders = [];
        foreach ($siglas_norte as $i => $sigla) {
...
// Função para processar os cases da Região Norte
function processar_norte($tituloNormalizado, &$descricao, &$dados, &$id_competicao, &$ano_inicio, &$exibir_tabela, &$tipo_participacao, &$tabela_estatisticas, $pdo, $coluna_ordem, $tipo_ordem) {
    $retorno = false; // Indica se algum case norte foi encontrado

    switch ($tituloNormalizado) {
        // Casos para Participações da Região Norte
        case 'participações do norte na libertadores':
            $descricao = "Participações dos clubes da Região Norte na Copa Libertadores da América.";
            $id_competicao = 5;
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_norte';
            $retorno = true;
            break;
        case 'participações do norte na sul americana':
            $descricao = "Participações dos clubes da Região Norte na Copa Sul Americana.";
            $id_competicao = 7;
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_norte';
            $retorno = true;
            break;
        case 'participações do norte - série a':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série A.";
            $id_competicao = 19; // Série A
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_norte';
            $retorno = true;
            break;
        case 'participações do norte - série a - pontos corridos':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série A 
            ...
            $retorno = true;
            break;
        case 'participações do norte - série b':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série B.";
            $id_competicao = 20; // Série B
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_norte';
            $retorno = true;
            break;
        case 'participações do norte - série b - pontos corridos':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série B 
            no período dos Pontos Corridos.";
            $id_competicao = 20;
            $ano_inicio = 2006;
            ...
            $retorno = true;
            break;
        case 'participações do norte - série c':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série C.";
            $id_competicao = 21; // Série C
            $exibir_tabela = true;
            $tipo_participacao = 'participacoes_norte';
            $retorno = true;
            break;
        case 'participações do norte - série d':
            $descricao = "Participações dos clubes da Região Norte no Campeonato Brasileiro Série D.";
        ...
        // Casos para Resultados da Região Norte (resultados gerais)
        case 'norte na libertadores':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Copa Libertadores da América.";
            $id_competicao = 5;
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'norte na sul americana':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Copa Sul Americana.";
            $id_competicao = 7;
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'norte na série a':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série A do Campeonato Brasileiro.";
            $id_competicao = 19; // Série A
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'norte na série a - pontos corridos':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série A do Campeonato Brasileiro 
            no período dos pontos corridos.";
            $id_competicao = 19;
            $ano_inicio = 2003;
            ...
            $retorno = true;
            break;
        case 'norte na série b':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série B do Campeonato Brasileiro.";
            $id_competicao = 20; // Série B
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'norte na série b - pontos corridos':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série B do Campeonato Brasileiro 
            no período dos pontos corridos.";
            $id_competicao = 20;
            $ano_inicio = 2006;
            ...
            $retorno = true;
            break;
        case 'norte na série c':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série C do Campeonato Brasileiro.";
            $id_competicao = 21; // Série C
            $exibir_tabela = true;
            $retorno = true;
            break;
        case 'norte na série d':
            $descricao = "Estatísticas gerais dos clubes da Região Norte na Série D do Campeonato Brasileiro.";
            $id_competicao = 22; // Série D
            $exibir_tabela = true;
            $retorno = true;
            break;
    ...
    // Executa a consulta específica para participações da Região Norte antes da consulta genérica de resultados
    if ($retorno && !empty($id_competicao) && $exibir_tabela && $tipo_participacao === 'participacoes_norte') {
        $tabela_estatisticas = build_sql_query_participacoes_norte($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem);
    }
    // Executa a consulta genérica apenas se for um case de resultados da Região Norte e não for do tipo participacoes
    elseif ($retorno && !empty($id_competicao) && $exibir_tabela && $tipo_participacao === null) {
        $tabela_estatisticas = build_sql_query_norte($pdo, $id_competicao, $ano_inicio, $tituloNormalizado, $coluna_ordem, $tipo_ordem);
    }

    return $retorno; // Retorna true se o item foi um case da Região Norte, false caso contrário
}
?>
